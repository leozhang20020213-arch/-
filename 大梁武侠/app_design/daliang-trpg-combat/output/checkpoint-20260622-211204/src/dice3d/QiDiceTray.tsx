...
export interface QiDiceTrayProps {
  ...
  onAssignToSlot?: (dieId: string, slot: "yin" | "yang") => void;
...
const DIE_REST_Y = 0.08;
const DRAG_LIFT_Y = 0.35;
...
const DIE_GEOM_SPECS: Record<string, { create: () => THREE.BufferGeometry; faceValues: number[] }> = {
  D4: {
    create: () => new THREE.TetrahedronGeometry(0.62, 0),
    faceValues: [1, 2, 3, 4],
  },
  D6: {
    create: () => new THREE.BoxGeometry(0.88, 0.88, 0.88, 1, 1, 1),
    faceValues: [1, 6, 3, 4, 2, 5],
  },
  D8: {
    create: () => new THREE.OctahedronGeometry(0.62, 0),
  ...
  D10: {
    create: () => {
      ...
      const n = 5;
      const top = new THREE.Vector3(0, 0.48, 0);
      const bot = new THREE.Vector3(0, -0.48, 0);
      const positions: number[] = [];
      const indices: number[] = [];
      for (let i = 0; i < n; i++) {
        const angle = (Math.PI * 2 * i) / n - Math.PI / 2;
        const r = 0.38;
        positions.push(Math.cos(angle) * r, 0.18, Math.sin(angle) * r);
        positions.push(Math.cos(angle + Math.PI / n) * r, -0.18, Math.sin(angle + Math.PI / n) * r);
      }
      for (let i = 0; i < n; i++) {
        const t0 = i * 2;
        const t1 = (i * 2 + 2) % (n * 2);
  ...
  D12: {
    create: () => new THREE.DodecahedronGeometry(0.4, 0),
    faceValues: Array.from({ length: 12 }, (_, i) => i + 1),
  },
  D20: {
    create: () => new THREE.IcosahedronGeometry(0.42, 0),
...
function createDieMaterial(nature: string): THREE.MeshStandardMaterial {
  const colors: Record<string, { body: number; roughness: number; metalness: number }> = {
    yin: { body: 0x151515, roughness: 0.62, metalness: 0.1 },
    yang: { body: 0xf8f4e8, roughness: 0.52, metalness: 0.08 },
    raw: { body: 0x1f5f9a, roughness: 0.56, metalness: 0.14 },
  };
...
function createFaceTexture(value: number, nature: string): THREE.CanvasTexture {
  ...
  ctx.fillStyle = nature === "yang" ? "rgba(255,255,255,0.42)" : "rgba(0,0,0,0.18)";
  ctx.beginPath();
...
function trayPosition(index: number, count: number, compact: boolean): THREE.Vector3 {
  ...
  const spacingX = compact ? 1.0 : 1.2;
  const spacingZ = compact ? 0.9 : 1.05;
...
function tempQiPosition(index: number, count: number, compact: boolean): THREE.Vector3 {
  const columns = Math.min(6, Math.max(1, count));
  const col = index % columns;
  const spacingX = compact ? 0.72 : 0.86;
  const x = (col - (columns - 1) / 2) * spacingX;
...
export function QiDiceTray({
  ...
  const onAssignToSlotRef = useRef(onAssignToSlot);
  ...
  const frustumSizeRef = useRef(compact ? 4.4 : 5.0);
  ...
  frustumSizeRef.current = compact ? 4.4 : 5.0;
  ...
  useEffect(() => {
    ...
    // Lighting — adjusted for top-down readability
    scene.add(new THREE.AmbientLight(0xffffff, 0.7));
    const dir = new THREE.DirectionalLight(0xffffff, 1.0);
    dir.position.set(2, 6, 3);
    scene.add(dir);
    const rim = new THREE.DirectionalLight(0xf2c14e, 0.4);
    ...
    function onPointerUp(event: PointerEvent) {
      ...
      if (drag.originalSlot === null) {
        ...
        if (isInZone(dropPos, YIN_SLOT_BOUNDS)) {
          onAssignToSlotRef.current?.(drag.dieId, "yin");
        } else if (isInZone(dropPos, YANG_SLOT_BOUNDS)) {
          onAssignToSlotRef.current?.(drag.dieId, "yang");
        } else {
          // Snap back
  ...
  function runPhysicsPhase(now: number) {
    ...
    for (let i = 0; i < physDice.length; i++) {
      for (let j = i + 1; j < physDice.length; j++) {
        ...
        const minDist = 0.74; // collision radius
    ...
    dice.forEach((die) => {
      ...
      if (edgeMat) {
        edgeMat.color = new THREE.Color(
          isSelected ? 0xffd76b : isHighlighted ? 0x88aacc : 0xd7bd76,
        );
        edgeMat.opacity = isSelected ? 1 : isHighlighted ? 0.85 : 0.45;
      }
